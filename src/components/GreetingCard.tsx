...
import { Sparkles, Stars, Music } from "lucide-react";
...
const GreetingCard = ({ name, className }: GreetingCardProps) => {
  ...
  const toggleMusic = () => {
    const audio = document.getElementById('eid-music') as HTMLAudioElement;
    if (audio) {
      if (isPlaying) {
        audio.pause();
      } else {
  ...
  return (
    <>
      <audio id="eid-music" loop preload="auto" className="hidden">
      ...
      <Card className={cn(
        ...
        <CardContent className="p-8 relative text-center">
          ...
          <div className="mt-8 text-base text-white/90 animate-fade-in">
            ...
            <p>Wishing you joy, health, and blessings on this auspicious occasion.</p>
          </div>
